...
// print helpers
/*
#define FATAL(x,args...)	do{printf("\033[%d;%d;%dm[FATAL]\033[0m "x,VT_BLINK,BG_BLACK,FG_RED,##args);printed_lines++;fatal_error=1;}while(0)
#define ERROR(x,args...) 	do{printf("\033[%d;%d;%dm[ERROR]\033[0m "x,VT_BRIGHT,BG_BLACK,FG_RED,##args);printed_lines++;}while(0)
#define WARN(x,args...)		do{printf("\033[%d;%dm[WARN ]\033[0m "x,BG_BLACK,FG_YELLOW,##args);printed_lines++;}while(0)
#define INFO(x,args...)		do{printf("\033[%d;%dm[INFO ]\033[0m "x,BG_BLACK,FG_GREEN,##args);printed_lines++;}while(0)
#ifdef DEVELOPMENT
#define DEBUG(x,args...) 	do{printf("\033[%d;%d;%dm[DEBUG]\033[0m "x,VT_BRIGHT,BG_BLACK,FG_MAGENTA,##args);printed_lines++;}while(0)
